...
dunto/privacy/index.html     → https://arachaaa.github.io/dunto/privacy/
dunto/support/index.html     → https://arachaaa.github.io/dunto/support/
```
...
| `dunto/privacy/` | App Store Connect 의 **Privacy Policy URL** · 앱 스토어 페이지에 공개 |
| `dunto/support/` | App Store Connect 의 **Support URL**(필수) · 앱 스토어 페이지의 「앱 지원」 링크 |

**Marketing URL(`dunto/`)은 아직 없다** — 선택 항목이라 제출을 막지 않는다.

> **두 앱의 방침은 내용이 크게 다르다** — pemvio 는 AdMob 배너가 있어 광고·ATT 절이 있고,
> **Dunto 는 네트워크 코드 자체가 없어** 「인터넷을 쓰지 않는다」가 핵심이다.
> 한쪽을 복사해 고치지 말고 **그 앱이 실제로 무엇을 하는지 코드로 확인하고 쓴다.**
